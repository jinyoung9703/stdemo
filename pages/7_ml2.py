...
# 멀티 페이지용 제목
st.set_page_config(page_title='Hello, machine learning!',
                   page_icon='💻')
...
pred = model.predict([[7.2,3.2,6.0,1.8]])
...
sl_val = st.slider('sepal length',iris.sepal_length.min(), iris.sepal_length.max(),
                      value=iris.sepal_length.mean())

sw_val = st.slider('sepal width',iris.sepal_width.min(), iris.sepal_width.max(),
                      value=iris.sepal_width.mean())

pl_val = st.slider('petal length',iris.petal_length.min(), iris.petal_length.max(),
                     value=iris.petal_length.mean())

pw_val = st.slider('petal width',iris.petal_width.min(), iris.petal_width.max(),
